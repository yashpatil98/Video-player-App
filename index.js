...
$(document).ready(function() {
    ...
    var frame = document.getElementById("video-player"); // to get the video src
    var playlist_Wrapper = document.getElementById("playlist-wrapper"); //get playlist div to append children
    for (var k = 0; k < playlistData.length; k++) { //iterate to add cards through function
        var card = createPlaylistCard(playlistData[k]); 
        if(card.id === "card1") { //adds the boundary to 1sd card on load
            card.classList = "active-card playlist-card";
            frame.src = "https://player.vimeo.com/video/190062231"; //adds 1st card src to video
        }
        playlist_Wrapper.appendChild(card);
    }


    var playlist_Card = $(".playlist-card"); //get the cards to change border ,class and get id
    var video_Player = $("iframe"); //get the iframe to change the src
    var views = $("#views-count"); //get the view count
    var title = $("#video-title"); //get the title
    var description = $("#video-description"); //get the description

    $(playlist_Card).click(function() { //everything happening on click
        for(var i = 0; i < playlist_Card.length; i++) { //iterate through card data
            $(playlist_Card[i]).removeClass();
            $(playlist_Card[i]).addClass("playlist-card") //1st removing boundary through class and adding default boundary
        }
        if($(this).attr("class") === "playlist-card active-card") { //if the boundary is yellow boundary is present then removing and then adding default
            $(this).removeClass();
            $(this).addClass("playlist-card");
        } else if ($(this).attr("class") === "playlist-card") { //if not then adding yellow boundary
            $(this).addClass("active-card");
        }

        for(var j = 0; j < videoPlaySectionData.length; j++) { //iterate through given video data to get vimeoId
            if($(this).attr("id") === "card" + videoPlaySectionData[j].id){ //matches the clicked card id with video id
                var vimeo = "https://player.vimeo.com/video/" + String(videoPlaySectionData[j].vimeoId);
                $(video_Player).attr("src", vimeo); //changing the src, title, views, description
                var viewsCount = videoPlaySectionData[j].views / 1000 ;
                $(views).text(viewsCount + "k");
                $(title).text(videoPlaySectionData[j].title);
                $(description).text(videoPlaySectionData[j].description);
            }
        }
    })
});